...
class CommandMediator(ICommandMediator):
    _command_register: dict[type[BaseCommand], CommandHandler] = field(
    ...
    def register_command(
        ...
        handler: CommandHandler[CommandT, CommandResultT],
    ) -> None:
...
class QueryMediator(IQueryMediator):
    _query_register: dict[type[BaseQuery], QueryHandler] = field(
        default_factory=dict, kw_only=True
    )

    def register_query(
        self, query_type: type[QueryT], handler: QueryHandler[QueryT, QueryResultT]
    ) -> None:
