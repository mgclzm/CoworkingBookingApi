...
def encode_access_token(sub: str) -> str:
    ...
    payload = {"sub": sub, "exp": expiration, "type": TokenType.ACCESS}
    ...
    async def validate(self, refresh_token_payload: dict[str, Any]) -> None:
        async with self.uow:
            ...
            found_refresh_tokens = (
                await self.uow.refresh_token_repository.find_all_by_user_id(user_id)
            )
            ...
            matching_token = next(
                (token for token in found_refresh_tokens if token.token_id == jti), None
            )
