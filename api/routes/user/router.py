...
@user_router.post(
    ...
    responses={
        ...
        status.HTTP_401_UNAUTHORIZED: {
            "description": "User is not registered, or the data provided is incorrect"
        },
    },
...
async def login(
    ...
    issue_refresh_token_command = IssueRefreshTokenCommand(
        form_data.username, form_data.password
    )
    try:
        response_schema = await command_mediator.execute_command(
            issue_refresh_token_command
        )
    except LogicException as ex:
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED, detail=ex.message
        ) from ex
    else:
        return response_schema
...
@user_router.post(
    ...
    responses={
        ...
        status.HTTP_401_UNAUTHORIZED: {
            "description": "Refresh token is not valid or not provided"
        },
    },
    summary="Endpoint to obtain new access token, require valid refresh token",
)
async def get_access_token(
    token_data: Annotated[RefreshTokenData, Depends(require_refresh_token)],
...
@user_router.get(
    ...
    responses={
        status.HTTP_200_OK: {
            "description": "Return information about current authenticated user"
        },
