...
from api.routes.user.security import (
    AuthException,
    CompositeRefreshTokenValidator,
    RefreshTokenClaimsValidator,
    RefreshTokenData,
    RefreshTokenJtiValidator,
...
async def require_refresh_token(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED, detail="Provided token is expired"
        )
    ...
    refresh_token_validator = CompositeRefreshTokenValidator(
        ...
        validators=[
            RefreshTokenTypeValidator(),
            RefreshTokenJtiValidator(typed_resolve(container, BaseUnitOfWork)),
        ],
    )
    try:
